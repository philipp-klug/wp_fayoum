...
function ecf_admin_head_script () {
	if ( strstr( $_SERVER['REQUEST_URI'], 'wp-admin/post-new.php' ) || strstr( $_SERVER['REQUEST_URI'], 'wp-admin/post.php' ) ) {
		if ( get_post_type( get_the_ID() ) == 'easycontactform' ) {
			...
			#ecfdemodiv .inside { max-height: 80px !important;}
			#ecfnewsvdiv .inside { max-height: 55px !important;}
			#ecfbuydiv .inside { max-height: 105px !important;}
...
// Config 	
		...
		'description' => __( '<span class="ecf-introjs"><a href="javascript:void(0);" onclick="startIntro();"><span class="ecf-intro-help"></span>Click Here to learn How to create your first Form</a></span><br /><br />You can add / remove, edit or order any elements with this form builder to fit to your needs.<br /><div class="ecfinfobox">Need Pro Version Features without <i>Full Upgrade</i>? <a href="'.admin_url( 'edit.php?post_type=easycontactform&page=ecf-addons' ).'">Check Available Addons here</a></div>', 'contact-form-lite' ),
...
<em>'.get_bloginfo('name').'</em>',
			...
			array(
					'name' => __( 'Captcha Public key', 'contact-form-lite' ),
					'desc' => __( 'Make sure to enter Captcha Secret key correctly to avoid email delivery failure.', 'contact-form-lite' ),
			...
			array(
					...
					'desc' => __( 'For best performance we recommend you to set less than 2M even though your server provides a limit of '.ini_get( 'upload_max_filesize' ).'. Set to 0 if you want to use default server Max Limit.', 'contact-form-lite' ),
